...
/*   Updated: 2019/04/29 12:55:54 by mjalenqu    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
char			*add_prec_to_pointer(t_data data, char *arg)
{
	...
	i = 0;
	j = 0;
	if ((size_t)ft_strlen(arg) > (size_t)data.prec)
		return (arg);
	...
	while ((i - 3 + (size_t)ft_strlen(arg)) <= (size_t)data.prec)
		new_arg[i++] = '0';
	while (arg[j])
		new_arg[i++] = arg[j++];
	free(arg);
...
char			*handle_width(t_data data, char *arg)
{
	...
	if (arg != NULL)
	{
		if (data.prec_dot == 1 && data.prec == 0 &&
			...
			while ((size_t)ft_strlen(arg) < (size_t)data.width)
...
char			*handle_prec(t_data data, char *arg)
{
	if (data.prec_dot == 1 && (data.conv_type >= 4 && data.conv_type <= 9))
	{
		...
		while ((size_t)ft_strlen(arg) < (size_t)data.prec)
			arg = add_char_begin_string(arg, "0");
